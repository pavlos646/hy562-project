...
def get_supporting_subgraph(sm: SummarizationManager, limit=10):
    ...
    pre_query = f"""
        ...
        RETURN
    """
    query = pre_query + return_statement(sm.node_labels, sm.default_properties)
    subgraph_df = execute_query(sm.spark, query)
    ...
    return subgraph_df, pre_query+" p"
...
def filter_graph_based_on_user(sm: SummarizationManager, interests, mode:Mode=Mode.LOOSE):
    ...
    for label, names in interests.items():
        ...
        conditions.append(f"(n:{label} AND toLower(n.{prop}) IN {formatted_names})")
    ...
    if mode in [Mode.STRICT, Mode.LOOSE]:
        ...
        m_where_clause = where_clause.replace('n:', 'm:').replace('toLower(n.', 'toLower(m.')
